...
import { POI_RESPONSE_POINTS } from "@/store/scoring";
...
export default function AddResponseModal({ isOpen, onClose, delegation }: AddResponseModalProps) {
  const addPOIResponse = useCommitteeStore((s) => s.addPOIResponse);
  const { addToast } = useToast();
  const [notes, setNotes] = useState("");

  const handleRate = (quality: QualityRating) => {
    addPOIResponse(delegation, quality, notes);
    const pts = POI_RESPONSE_POINTS[quality];
    addToast(`POI Response: ${quality} (+${pts} pts) — ${delegation}`);
    setNotes("");
    onClose();
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Add POI Response — ${delegation}`}>
      <p className="text-sm text-muted mb-4">Rate the quality of the response to a point of information.</p>
      <div className="grid grid-cols-2 gap-3 mb-5">
        {RATINGS.map((r) => (
          ...
          >
            <span className="text-lg font-bold">{POI_RESPONSE_POINTS[r.value]}</span>
